 ...
 * 
 * <p>Shaders that use the new per-view outputs (e.g. gl_PositionPerViewNV) <b>must</b> also write the non-per-view output (gl_Position), and the values written <b>must</b> be such that gl_Position = gl_PositionPerViewNV[gl_ViewIndex] for all views in the subpass. Implementations are free to either use the per-view outputs or the non-per-view outputs, whichever would be more efficient.</p>
 * 
 * <p>If {@code VK_NV_viewport_array2} is not also supported and enabled, the per-view viewport mask <b>must</b> not be used.</p></dd>
 * </dl>
 * 
 * <h5>Examples</h5>
 * 
