 ...
 * </dl>
 * 
 * <p>The {@code VK_MVK_macos_surface} extension is an instance extension. It provides a mechanism to create a {@code VkSurfaceKHR} object (defined by the {@code VK_KHR_surface} extension) that refers to an {@code NSView}, the native surface type of macOS, which is underpinned by a {@code CAMetalLayer}, to support rendering to the surface using Apple's Metal framework.</p>
 */
public class MVKMacosSurface {
	 ...
	 * <li>{@code pCreateInfo} <b>must</b> be a pointer to a valid {@link VkMacOSSurfaceCreateInfoMVK} structure</li>
