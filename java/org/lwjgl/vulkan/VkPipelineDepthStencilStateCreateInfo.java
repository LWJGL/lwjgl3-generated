...
public class VkPipelineDepthStencilStateCreateInfo extends Struct {
	...
	static {
		Layout layout = __struct(
			...
			__member(4),
			__member(4),
			__member(VkStencilOpState.SIZEOF, VkStencilOpState.__ALIGNMENT),
			__member(VkStencilOpState.SIZEOF, VkStencilOpState.__ALIGNMENT),
			__member(4),
			__member(4)
		);
	...
	/** {@link ByteBuffer} version of {@link #set(VkPipelineDepthStencilStateCreateInfo) set}. */
	public VkPipelineDepthStencilStateCreateInfo set(ByteBuffer struct) {
		if ( CHECKS )
