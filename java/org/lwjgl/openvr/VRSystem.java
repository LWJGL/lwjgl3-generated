...
public class VRSystem {
    ...
    public static void nVRSystem_GetOutputDevice(long pnDevice, int textureType) {
        long __functionAddress = OpenVR.VRSystem.GetOutputDevice;
        if (CHECKS) {
            check(__functionAddress);
        }
        callPV(__functionAddress, pnDevice, textureType);
    }
     ...
     * <h3>Vulkan Only</h3>
     * 
     * <p>Returns the {@code vk::PhysicalDevice} that should be used by the application.</p>
     * 
     * <h3>macOS Only</h3>
     * 
     * <p>Returns an {@code id<MTLDevice>} that should be used by the application.</p>
     *
     * @param pnDevice    
     * @param textureType 
     */
    public static void VRSystem_GetOutputDevice(LongBuffer pnDevice, int textureType) {
        if (CHECKS) {
            check(pnDevice, 1);
        }
        nVRSystem_GetOutputDevice(memAddress(pnDevice), textureType);
    }
