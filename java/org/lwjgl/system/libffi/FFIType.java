...
public class FFIType extends Struct {
	...
	/** Returns a new {@link FFIType} instance allocated with {@link MemoryUtil#memAlloc memAlloc}. The instance must be explicitly freed. */
	public static FFIType malloc() {
		return create(nmemAlloc(SIZEOF));
	}

	/** Returns a new {@link FFIType} instance allocated with {@link MemoryUtil#memCalloc memCalloc}. The instance must be explicitly freed. */
	...
	/** Returns a new {@link FFIType} instance allocated on the thread-local {@link MemoryStack}. */
	public static FFIType mallocStack() {
		return mallocStack(stackGet());
	}

	/** Returns a new {@link FFIType} instance allocated on the thread-local {@link MemoryStack} and initializes all its bits to zero. */
	public static FFIType callocStack() {
		return callocStack(stackGet());
	}

	/**
	 * Returns a new {@link FFIType} instance allocated on the specified {@link MemoryStack}.
	 *
	...
	/**
	 * Returns a new {@link FFIType} instance allocated on the specified {@link MemoryStack} and initializes all its bits to zero.
	 *
	 * @param stack the stack from which to allocate
	 */
	...
	public static final class Buffer extends StructBuffer<FFIType, Buffer> {
		...
		protected FFIType newInstance(long address) {
			return new FFIType(address, getContainer());
		}
