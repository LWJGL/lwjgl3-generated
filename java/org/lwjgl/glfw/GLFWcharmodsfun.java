...
public abstract class GLFWcharmodsfun extends Closure.Void {
	...
	static {
		...
		if ( status != FFI_OK )
			throw new IllegalStateException(String.format("Failed to prepare GLFWcharmodsfun callback interface. Status: 0x%X", status));
	}

	protected GLFWcharmodsfun() {
		super(CIF);
	}
	...
	/** A functional interface for {@link GLFWcharmodsfun}. */
