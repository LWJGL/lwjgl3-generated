...
public abstract class GLFWcursorposfun extends Closure.Void {
	...
	static {
		...
		if ( status != FFI_OK )
			throw new IllegalStateException(String.format("Failed to prepare GLFWcursorposfun callback interface. Status: 0x%X", status));
	}

	protected GLFWcursorposfun() {
		super(CIF);
	}
	...
	protected void callback(long args) {
		invoke(
			...
			memGetDouble(POINTER_SIZE * 1 + args),
			memGetDouble(POINTER_SIZE * 2 + args)
		);
	}
	/**
	...
	/** A functional interface for {@link GLFWcursorposfun}. */
