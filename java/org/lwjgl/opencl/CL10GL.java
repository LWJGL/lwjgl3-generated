...
public final class CL10GL {
	...
	/** JNI method for {@link #clCreateFromGLBuffer} */
	 ...
	 *
	 * @return a valid non-zero OpenCL buffer object and {@code errcode_ret} is set to {@link CL10#CL_SUCCESS} if the buffer object is created successfully. Otherwise, it returns a {@code NULL}
	 ...
	 *         <li>{@link CL10#CL_INVALID_VALUE} if values specified in {@code flags} are not valid.</li>
	 *         <li>{@link #CL_INVALID_GL_OBJECT} if {@code bufobj} is not a GL buffer object or is a GL buffer object but does not have an existing data store or the
	...
	public static CLMem clCreateFromGLBuffer(CLContext context, long flags, int bufobj, ByteBuffer errcode_ret) {
		long __functionAddress = getInstance(context).clCreateFromGLBuffer;
		if ( LWJGLUtil.CHECKS ) {
	...
	/** Alternative version of: {@link #clCreateFromGLBuffer} */
	...
	/** JNI method for {@link #clCreateFromGLTexture2D} */
	 ...
	 * @param miplevel       the mipmap level to be used. Implementations may return {@link CL10#CL_INVALID_OPERATION} for {@code miplevel} values &gt; 0.
	...
	public static CLMem clCreateFromGLTexture2D(CLContext context, long flags, int texture_target, int miplevel, int texture, ByteBuffer errcode_ret) {
		long __functionAddress = getInstance(context).clCreateFromGLTexture2D;
		if ( LWJGLUtil.CHECKS ) {
			checkFunctionAddress(__functionAddress);
			if ( errcode_ret != null ) checkBuffer(errcode_ret, 1 << 2);
		}
	...
	/** Alternative version of: {@link #clCreateFromGLTexture2D} */
	...
	/** JNI method for {@link #clCreateFromGLTexture3D} */
	 ...
	 * @param context        a valid OpenCL context created from an OpenGL context
	 * @param flags          a bit-field that is used to specify usage information. One of:<p/>{@link CL10#CL_MEM_READ_ONLY}, {@link CL10#CL_MEM_WRITE_ONLY}, {@link CL10#CL_MEM_READ_WRITE}
	 * @param texture_target defines the image type of {@code texture}. No reference to a bound GL texture object is made or implied by this parameter. Must be:<p/>{@link GL12#GL_TEXTURE_3D}
	 ...
	 * @param errcode_ret    will return an appropriate error code. If {@code errcode_ret} is {@code NULL}, no error code is returned.
	 *
	 * @return a valid non-zero image object and {@code errcode_ret} is set to {@link CL10#CL_SUCCESS} if the image object is created successfully. Otherwise, it returns a {@code NULL} value
	...
	public static CLMem clCreateFromGLTexture3D(CLContext context, long flags, int texture_target, int miplevel, int texture, ByteBuffer errcode_ret) {
		long __functionAddress = getInstance(context).clCreateFromGLTexture3D;
		if ( LWJGLUtil.CHECKS ) {
			checkFunctionAddress(__functionAddress);
			if ( errcode_ret != null ) checkBuffer(errcode_ret, 1 << 2);
		}
	...
	/** Alternative version of: {@link #clCreateFromGLTexture3D} */
	...
	/** JNI method for {@link #clCreateFromGLRenderbuffer} */
	 ...
	 *         <li>{@link CL10#CL_INVALID_CONTEXT} if {@code context} is not a valid context or was not created from a GL context.</li>
	 *         <li>{@link CL10#CL_INVALID_VALUE} if values specified in {@code flags} are not valid.</li>
	 *         <li>{@link #CL_INVALID_GL_OBJECT} if {@code renderbuffer} is not a GL renderbuffer object or if the width or height of {@code renderbuffer} is zero.</li>
	 *         <li>{@link CL10#CL_INVALID_IMAGE_FORMAT_DESCRIPTOR} if the OpenGL renderbuffer internal format does not map to a supported OpenCL image format.</li>
	 *         <li>{@link CL10#CL_INVALID_OPERATION} if {@code renderbuffer} is a multi-sample GL renderbuffer object.</li>
	 *         <li>{@link CL10#CL_OUT_OF_RESOURCES} if there is a failure to allocate resources required by the OpenCL implementation on the device.</li>
	 *         <li>{@link CL10#CL_OUT_OF_HOST_MEMORY} if there is a failure to allocate resources required by the OpenCL implementation on the host.</li>
	 *         </ul>
	 */
	public static CLMem clCreateFromGLRenderbuffer(CLContext context, long flags, int renderbuffer, ByteBuffer errcode_ret) {
		long __functionAddress = getInstance(context).clCreateFromGLRenderbuffer;
		if ( LWJGLUtil.CHECKS ) {
			checkFunctionAddress(__functionAddress);
			if ( errcode_ret != null ) checkBuffer(errcode_ret, 1 << 2);
		}
	...
	/** Alternative version of: {@link #clCreateFromGLRenderbuffer} */
	...
	/** JNI method for {@link #clGetGLObjectInfo} */
	 ...
	 * @param gl_object_type returns the type of GL object attached to {@code memobj}. If {@code gl_object_type} is {@code NULL}, it is ignored. One of:<p/>{@link #CL_GL_OBJECT_BUFFER}, {@link #CL_GL_OBJECT_TEXTURE2D}, {@link #CL_GL_OBJECT_TEXTURE3D}, {@link #CL_GL_OBJECT_RENDERBUFFER}, {@link CL12GL#CL_GL_OBJECT_TEXTURE_2D_ARRAY}, {@link CL12GL#CL_GL_OBJECT_TEXTURE1D}, {@link CL12GL#CL_GL_OBJECT_TEXTURE1D_ARRAY}, {@link CL12GL#CL_GL_OBJECT_TEXTURE_BUFFER}
	 * @param gl_object_name the GL object name used to create {@code memobj}. If {@code gl_object_name} is {@code NULL}, it is ignored.
	 *
	 * @return {@link CL10#CL_SUCCESS} if the call was executed successfully. Otherwise, it returns one of the following errors:
	 *         <ul>
	 *         <li>{@link CL10#CL_INVALID_MEM_OBJECT} if {@code memobj} is not a valid OpenCL memory object.</li>
	 *         <li>{@link #CL_INVALID_GL_OBJECT} if there is no GL object associated with {@code memobj}.</li>
	 *         <li>{@link CL10#CL_OUT_OF_RESOURCES} if there is a failure to allocate resources required by the OpenCL implementation on the device.</li>
	 *         <li>{@link CL10#CL_OUT_OF_HOST_MEMORY} if there is a failure to allocate resources required by the OpenCL implementation on the host.</li>
	 *         </ul>
	 */
	public static int clGetGLObjectInfo(CLMem memobj, ByteBuffer gl_object_type, ByteBuffer gl_object_name) {
		long __functionAddress = getInstance(memobj).clGetGLObjectInfo;
		if ( LWJGLUtil.CHECKS )
			checkFunctionAddress(__functionAddress);
	...
	/** Alternative version of: {@link #clGetGLObjectInfo} */
	...
	/** JNI method for {@link #clGetGLTextureInfo} */
	 ...
	 *         <ul>
	 *         <li>{@link CL10#CL_INVALID_MEM_OBJECT} if {@code memobj} is not a valid OpenCL memory object.</li>
	 *         <li>{@link #CL_INVALID_GL_OBJECT} if there is no GL texture object associated with {@code memobj}.</li>
	 *         <li>{@link CL10#CL_INVALID_VALUE} if {@code param_name} is not valid, or if size in bytes specified by {@code param_value_size} is &lt; size of return
	...
	public static int clGetGLTextureInfo(CLMem memobj, int param_name, long param_value_size, ByteBuffer param_value, ByteBuffer param_value_size_ret) {
		long __functionAddress = getInstance(memobj).clGetGLTextureInfo;
		if ( LWJGLUtil.CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	/** Alternative version of: {@link #clGetGLTextureInfo} */
	...
	/** IntBuffer version of: {@link #clGetGLTextureInfo} */
	...
	/** JNI method for {@link #clEnqueueAcquireGLObjects} */
	 ...
	 *
	 * @return {@link CL10#CL_SUCCESS} if the function is executed successfully. If {@code num_objects} is 0 and {@code mem_objects} is {@code NULL} the function does nothing and returns
	 *         {@link CL10#CL_SUCCESS}. Otherwise, it returns one of the following errors:
	 *         <ul>
	 *         <li>{@link CL10#CL_INVALID_VALUE} if {@code num_objects} is zero and {@code mem_objects} is not a {@code NULL} value or if {@code num_objects} &gt; 0 and
	 *         {@code mem_objects} is {@code NULL}.</li>
	 *         <li>{@link CL10#CL_INVALID_MEM_OBJECT} if memory objects in {@code mem_objects} are not valid OpenCL memory objects.</li>
	 *         <li>{@link CL10#CL_INVALID_COMMAND_QUEUE} if {@code command_queue} is not a valid command-queue.</li>
	 *         <li>{@link CL10#CL_INVALID_CONTEXT} if context associated with {@code command_queue} was not created from an OpenGL context.</li>
	 *         <li>{@link #CL_INVALID_GL_OBJECT} if memory objects in {@code mem_objects} have not been created from a GL object(s).</li>
	...
	public static int clEnqueueAcquireGLObjects(CLCommandQueue command_queue, int num_objects, ByteBuffer mem_objects, int num_events_in_wait_list, ByteBuffer event_wait_list, ByteBuffer event) {
		long __functionAddress = getInstance(command_queue).clEnqueueAcquireGLObjects;
		if ( LWJGLUtil.CHECKS ) {
			checkFunctionAddress(__functionAddress);
	...
	/** Alternative version of: {@link #clEnqueueAcquireGLObjects} */
	...
	/** Single value version of: {@link #clEnqueueAcquireGLObjects} */
	...
	/** JNI method for {@link #clEnqueueReleaseGLObjects} */
	...
	public static int clEnqueueReleaseGLObjects(CLCommandQueue command_queue, int num_objects, ByteBuffer mem_objects, int num_events_in_wait_list, ByteBuffer event_wait_list, ByteBuffer event) {
		long __functionAddress = getInstance(command_queue).clEnqueueReleaseGLObjects;
		if ( LWJGLUtil.CHECKS ) {
			checkFunctionAddress(__functionAddress);
			checkBuffer(mem_objects, num_objects << POINTER_SHIFT);
			if ( event_wait_list != null ) checkBuffer(event_wait_list, num_events_in_wait_list << POINTER_SHIFT);
	...
	/** Alternative version of: {@link #clEnqueueReleaseGLObjects} */
	...
	/** Single value version of: {@link #clEnqueueReleaseGLObjects} */
