...
/** Contains OpenGL-specific rendering information on Windows. */
public final class OVRGLConfigWindows implements Pointer {
	...
	public OVRGLConfigWindows() {
		this(malloc());
	}

	public OVRGLConfigWindows(ByteBuffer struct) {
		if ( LWJGLUtil.CHECKS )
	...
	public void setOGLWindow(long Window) { OGLWindow(struct, Window); }
	public void setOGLDC(long DC) { OGLDC(struct, DC); }
	...
	public long getOGLWindow() { return OGLWindow(struct); }
	public long getOGLDC() { return OGLDC(struct); }
	...
	public static void OGLSet(ByteBuffer ovrglconfig, ByteBuffer ogl) { if ( ogl != null ) memCopy(memAddress(ogl), memAddress(ovrglconfig) + OGL, OVRGLConfigDataWindows.SIZEOF); }
	public static void OGLHeaderSet(ByteBuffer ovrglconfig, ByteBuffer Header) { if ( Header != null ) memCopy(memAddress(Header), memAddress(ovrglconfig) + OGL + OVRGLConfigDataWindows.HEADER, OVRRenderAPIConfigHeader.SIZEOF); }
	public static void OGLHeaderAPI(ByteBuffer ovrglconfig, int API) { ovrglconfig.putInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.API, API); }
	public static void OGLHeaderBackBufferSizeSet(ByteBuffer ovrglconfig, ByteBuffer BackBufferSize) { if ( BackBufferSize != null ) memCopy(memAddress(BackBufferSize), memAddress(ovrglconfig) + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.BACKBUFFERSIZE, OVRSizei.SIZEOF); }
	public static void OGLHeaderBackBufferSizeW(ByteBuffer ovrglconfig, int w) { ovrglconfig.putInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.BACKBUFFERSIZE + OVRSizei.W, w); }
	public static void OGLHeaderBackBufferSizeH(ByteBuffer ovrglconfig, int h) { ovrglconfig.putInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.BACKBUFFERSIZE + OVRSizei.H, h); }
	public static void OGLHeaderMultisample(ByteBuffer ovrglconfig, int Multisample) { ovrglconfig.putInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.MULTISAMPLE, Multisample); }
	public static void OGLWindow(ByteBuffer ovrglconfig, long Window) { PointerBuffer.put(ovrglconfig, ovrglconfig.position() + OGL + OVRGLConfigDataWindows.WINDOW, Window); }
	public static void OGLDC(ByteBuffer ovrglconfig, long DC) { PointerBuffer.put(ovrglconfig, ovrglconfig.position() + OGL + OVRGLConfigDataWindows.DC, DC); }
	...
	public static void OGLGet(ByteBuffer ovrglconfig, ByteBuffer ogl) { checkBuffer(ogl, OVRGLConfigDataWindows.SIZEOF); memCopy(memAddress(ovrglconfig) + OGL, memAddress(ogl), OVRGLConfigDataWindows.SIZEOF); }
	public static void OGLHeaderGet(ByteBuffer ovrglconfig, ByteBuffer Header) { checkBuffer(Header, OVRRenderAPIConfigHeader.SIZEOF); memCopy(memAddress(ovrglconfig) + OGL + OVRGLConfigDataWindows.HEADER, memAddress(Header), OVRRenderAPIConfigHeader.SIZEOF); }
	public static int OGLHeaderAPI(ByteBuffer ovrglconfig) { return ovrglconfig.getInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.API); }
	public static void OGLHeaderBackBufferSizeGet(ByteBuffer ovrglconfig, ByteBuffer BackBufferSize) { checkBuffer(BackBufferSize, OVRSizei.SIZEOF); memCopy(memAddress(ovrglconfig) + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.BACKBUFFERSIZE, memAddress(BackBufferSize), OVRSizei.SIZEOF); }
	public static int OGLHeaderBackBufferSizeW(ByteBuffer ovrglconfig) { return ovrglconfig.getInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.BACKBUFFERSIZE + OVRSizei.W); }
	public static int OGLHeaderBackBufferSizeH(ByteBuffer ovrglconfig) { return ovrglconfig.getInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.BACKBUFFERSIZE + OVRSizei.H); }
	public static int OGLHeaderMultisample(ByteBuffer ovrglconfig) { return ovrglconfig.getInt(ovrglconfig.position() + OGL + OVRGLConfigDataWindows.HEADER + OVRRenderAPIConfigHeader.MULTISAMPLE); }
	public static long OGLWindow(ByteBuffer ovrglconfig) { return PointerBuffer.get(ovrglconfig, ovrglconfig.position() + OGL + OVRGLConfigDataWindows.WINDOW); }
	public static long OGLDC(ByteBuffer ovrglconfig) { return PointerBuffer.get(ovrglconfig, ovrglconfig.position() + OGL + OVRGLConfigDataWindows.DC); }
