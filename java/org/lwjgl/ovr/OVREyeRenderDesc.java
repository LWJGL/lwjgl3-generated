...
public final class OVREyeRenderDesc implements Pointer {
	...
	public static void FovGet(ByteBuffer ovreyerenderdesc, ByteBuffer Fov) { checkBuffer(Fov, OVRFovPort.SIZEOF); memCopy(memAddress(ovreyerenderdesc) + FOV, memAddress(Fov), OVRFovPort.SIZEOF); }
	...
	public static void DistortedViewportGet(ByteBuffer ovreyerenderdesc, ByteBuffer DistortedViewport) { checkBuffer(DistortedViewport, OVRRecti.SIZEOF); memCopy(memAddress(ovreyerenderdesc) + DISTORTEDVIEWPORT, memAddress(DistortedViewport), OVRRecti.SIZEOF); }
	public static void DistortedViewportPosGet(ByteBuffer ovreyerenderdesc, ByteBuffer Pos) { checkBuffer(Pos, OVRVector2i.SIZEOF); memCopy(memAddress(ovreyerenderdesc) + DISTORTEDVIEWPORT + OVRRecti.POS, memAddress(Pos), OVRVector2i.SIZEOF); }
	...
	public static void DistortedViewportSizeGet(ByteBuffer ovreyerenderdesc, ByteBuffer Size) { checkBuffer(Size, OVRSizei.SIZEOF); memCopy(memAddress(ovreyerenderdesc) + DISTORTEDVIEWPORT + OVRRecti.SIZE, memAddress(Size), OVRSizei.SIZEOF); }
	...
	public static void PixelsPerTanAngleAtCenterGet(ByteBuffer ovreyerenderdesc, ByteBuffer PixelsPerTanAngleAtCenter) { checkBuffer(PixelsPerTanAngleAtCenter, OVRVector2f.SIZEOF); memCopy(memAddress(ovreyerenderdesc) + PIXELSPERTANANGLEATCENTER, memAddress(PixelsPerTanAngleAtCenter), OVRVector2f.SIZEOF); }
	...
	public static void HmdToEyeViewOffsetGet(ByteBuffer ovreyerenderdesc, ByteBuffer HmdToEyeViewOffset) { checkBuffer(HmdToEyeViewOffset, OVRVector3f.SIZEOF); memCopy(memAddress(ovreyerenderdesc) + HMDTOEYEVIEWOFFSET, memAddress(HmdToEyeViewOffset), OVRVector3f.SIZEOF); }
